...
Submit the typed completion as your final action. For every task return exactly one taskCompletions entry: use kind \`checkpoint\` with the reachable commit SHA that covers it (several tasks may use the same cumulative SHA), or kind \`already_satisfied\` with concrete repository-state evidence. If any tracked work changed, return outcome \`changed\` and candidateTip equal to the final committed HEAD. If all tasks were already satisfied, return outcome \`already_satisfied\` and omit candidateTip. Include verification evidence and uncertainty when applicable.`;
}
...
export function buildRecoveryPrompt(packet: RecoveryWorkerPacket): string {
  ...
  const scopeInstructions =
    packet.workspace.scope === "runtime"
      ? "This is a runtime-scoped hook turn: use only retry, repair_environment, diagnose, or no_safe_action. Do not make tracked candidate corrections. A diagnosis can advance a later turn to the candidate workspace."
      : "This is a candidate-scoped turn: use only retry, rework_candidate, reconcile, recreate_workspace, diagnose, or no_safe_action. Commit every tracked correction in this one assigned worktree before reporting it.";
  return `You are the Pipkin Implement recovery agent. Recover only the validated failure packet below.\n\nAssigned recovery workspace:\n\n  ${packet.workspace.path}\n\nThe target checkout and immutable source corpus are orchestrator-owned. Do not access or mutate them. ${scopeInstructions} Do not push, rewrite history, bypass hooks, change protected plan artifacts, or leave an active Git operation or uncommitted work.\n\nTarget identity: ${packet.target.branchRef} @ ${packet.target.startHead}\nPermitted mutation boundary: ${packet.workspace.mutationBoundary}\nCurrent candidate: ${JSON.stringify(packet.candidate ?? null, null, 2)}\n\n## Failed gate evidence\n\n${JSON.stringify(packet.gate, null, 2)}\n\n## Recovery episode\n\n${JSON.stringify(packet.episode, null, 2)}\n\n## Complete outstanding findings\n\n${formatFindings(packet.outstandingFindings)}${provenance}\n\nChoose exactly one typed action. Use \`retry\` only when the retained candidate/workspace identity is unchanged and the failed lifecycle gate can be rerun. Use \`repair_environment\` only for ignored/runtime repair. Use \`recreate_workspace\` only when a trusted checkpoint is retained. Use \`rework_candidate\` or \`reconcile\` only after committing a tracked correction in the assigned candidate workspace; include candidateTip and the changed paths. Use \`diagnose\` only to retain a bounded diagnosis before another recovery turn. Use \`no_safe_action\` when no safe bounded action exists. Include concrete evidence for every action.`;
}
