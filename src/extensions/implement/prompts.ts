...
Submit the typed completion as your final action. Return outcome \`changed\` only when you have left a committed candidate that changes the workstream tree. For a changed outcome, provide a concise Conventional Commit message that describes the cumulative workstream change, not merely the latest checkpoint. Return outcome \`already_satisfied\` only when every contract was already satisfied and you made no commits; include concrete repository-state evidence for that outcome. Include at least one concise verification statement describing what you checked and the outcome. Verification may use tests, static analysis, direct inspection, or other appropriate evidence; it does not require a shell command. Include uncertainty when applicable.`;
}
...
export function buildRecoveryPrompt(packet: RecoveryWorkerPacket): string {
  const provenance = packet.gate.artifactProvenance
    ...
    : "";
  const stagingComparison = packet.episode.workspace.stagingComparison
    ? `\n\n## Retained failed staging state\n\nBase SHA: ${packet.episode.workspace.stagingComparison.baseSha}\nStaged tree: ${packet.episode.workspace.stagingComparison.treeSha}\n\nInspect it with git diff --stat ${packet.episode.workspace.stagingComparison.baseSha} ${packet.episode.workspace.stagingComparison.treeSha} and git diff --name-status ${packet.episode.workspace.stagingComparison.baseSha} ${packet.episode.workspace.stagingComparison.treeSha}, then use git diff ${packet.episode.workspace.stagingComparison.baseSha} ${packet.episode.workspace.stagingComparison.treeSha} -- <path> for scoped paths. Do not dump the full range into context.`
    : "";
  return `You are the Pipkin Implement recovery agent. Recover only the validated failure packet below.\n\nAssigned recovery workspace:\n\n  ${packet.workspace.path}\n\nThe target checkout and immutable source corpus are orchestrator-owned. Do not access or mutate them. Commit every tracked correction in this one assigned candidate worktree before reporting it. Do not push, rewrite history, bypass hooks, change protected plan artifacts, or leave an active Git operation or uncommitted work.\n\nTarget identity: ${packet.target.branchRef} @ ${packet.target.startHead}\nPermitted mutation boundary: ${packet.workspace.mutationBoundary}\nCurrent candidate: ${JSON.stringify(packet.candidate ?? null, null, 2)}\n\n## Failed gate evidence\n\n${JSON.stringify(packet.gate, null, 2)}\n\n## Recovery episode\n\n${JSON.stringify(packet.episode, null, 2)}\n\n## Complete outstanding findings\n\n${formatFindings(packet.outstandingFindings)}${stagingComparison}${provenance}\n\nChoose exactly one typed action. Use \`retry\` only when the retained candidate/workspace identity is unchanged and the failed lifecycle gate can be rerun. Use \`recreate_workspace\` only when a trusted checkpoint is retained. Use \`rework_candidate\` or \`reconcile\` only after committing a tracked correction in the assigned candidate workspace; include candidateTip, the changed paths, and a concise Conventional Commit message describing the complete candidate that would be published, not merely the latest correction. Use \`diagnose\` only to retain a bounded diagnosis before another recovery turn. Use \`no_safe_action\` when no safe bounded action exists. Include concrete evidence for every action.`;
}
