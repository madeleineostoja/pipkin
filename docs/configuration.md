...
BTW uses the model associated with the current conversation flow. Explicit `Agent` tool arguments override a public agent's model or thinking for that invocation only; they are not saved.
